    ...
    private void Draw()
    {
        ...
        if (Physics.Raycast(TopLeft, transform.up, out _touchTop, _tipHeight))
        {
            ...
            if (_touchTop.transform.CompareTag("Whiteboard"))
            {
                ...
                if (x1 < 0)
                    x1 = 0;
                else if (x1 > _whiteboard.textureSize.x)
                    x1 = (int)_whiteboard.textureSize.x;
                if (y1 < 0)
                    y1 = 0;
                else if (y1 > _whiteboard.textureSize.y)
                    y1 = (int)_whiteboard.textureSize.y;
                if (x2 < 0)
                    x2 = 0;
                else if (x2 > _whiteboard.textureSize.x)
                    x2 = (int)_whiteboard.textureSize.x;
                if (y2 < 0)
                    y2 = 0;
                else if (y2 > _whiteboard.textureSize.y)
                    y2 = (int)_whiteboard.textureSize.y;
                if (x1 < x2)
                {
                    var temp = x2;
                    x2 = x1;
                ...
                if (_touchedLastFrame)
                {
                    ...
                    if (TestColourToggle)
                        _colours = Enumerable.Repeat(TestColour, (int)(eraseSize.x * eraseSize.y)).ToArray();
                    else
                        _colours = Enumerable.Repeat(_whiteboard.Colour, (int)(eraseSize.x * eraseSize.y)).ToArray();
